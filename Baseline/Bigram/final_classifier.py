...
# Save the model
torch.save(model.state_dict(), 'trump_classifier_model.pth')
...
# 10. Generating High-Quality Plot
plt.figure(figsize=(10, 6))
plt.plot(train_losses, label='Training Loss', linewidth=2)
plt.plot(val_losses, label='Validation Loss', linewidth=2)
plt.title('LSTM Model Learning Curve', fontsize=14)
plt.xlabel('Epochs', fontsize=12)
plt.ylabel('Loss (Binary Cross Entropy)', fontsize=12)
plt.legend()
plt.grid(True, linestyle='--', alpha=0.7)
plt.savefig('learning_curve.png', dpi=300, bbox_inches='tight')
print("High-quality plot saved as 'learning_curve.png'.")

# 11. Saving Statistical Report
report_content = f"""
TRUMP TWEET CLASSIFIER - STATISTICAL REPORT
-------------------------------------------
Dataset Statistics:
- Total Samples: {len(data)}
- Real Tweets: {len(real_tweets)}
- Fake Tweets (Bigram): {len(fake_tweets)}
...
with open('classification_stats.txt', 'w', encoding='utf-8') as f:
    f.write(report_content)
print("Statistical report saved as 'classification_stats.txt'.")