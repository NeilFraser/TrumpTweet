...
# 4. Final Model Architecture (Bi-LSTM + Spatial Dropout)
class TrumpClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # Spatial dropout requires (Batch, Channel, Length)
...
# 6. Training Loop
print("Starting Training (Bi-LSTM + Spatial Dropout)...")
...
torch.save(model.state_dict(), 'trump_classifier_final_model.pth')
print("\n[SUCCESS] Model saved as 'trump_classifier_final_model.pth'")
...
plt.title('Learning Curve: Bi-LSTM + Spatial Dropout (Trigram)', fontsize=14)
plt.xlabel('Epochs', fontsize=12)
plt.ylabel('Loss', fontsize=12)
plt.legend()
plt.grid(True, linestyle='--', alpha=0.6)
plt.savefig('final_learning_curve.png', dpi=300)
print("[SUCCESS] Learning curve saved as 'final_learning_curve.png'")