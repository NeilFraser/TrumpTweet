...
# 6. Model Architecture (LSTM with Lower Dropout)
class TrumpClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim):
        super(TrumpClassifier, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        
        # Lowering dropout to 0.2 to avoid losing too much information
        self.dropout = nn.Dropout(0.2) 
        
        # num_layers=2 helps the model learn more complex patterns in Trigrams
    ...
    def forward(self, x):
        ...
        # Forward pass through the 2-layer LSTM
        _, (hidden, _) = self.lstm(embedded)
        
        # We use the hidden state of the last layer (hidden[-1])
        # and apply dropout one last time before the final decision
...
print("Test 1.1: Starting training with 2-layer LSTM and LOW Dropout (0.2)...")
# 8. Training Loop
